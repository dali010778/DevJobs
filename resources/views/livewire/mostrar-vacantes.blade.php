<div>
    <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
        ...
        @forelse ($vacantes as $vacante )
            <div class="p-6 bg-white border-b text-gray-700 md:flex md:justify-between
                md:items-center">
                <div class="space-y-3">
                ...
                <div class="flex flex-col md:flex-row items-stretch gap-3  mt-5 md:mt-0">
                    ...
                    <a href="{{ route('vacantes.edit',$vacante->id) }}" class="bg-blue-800 py-2 px-4 rounded-lg text-white text-xs font-bold
                        uppercase text-center">Editar
                    </a>

                    <button wire:click="$emit('mostrarAlerta', {{ $vacante->id }})" class="bg-red-800 py-2 px-4 rounded-lg text-white text-xs font-bold
                        uppercase text-center">Eliminar
                    </button>
                </div>
            </div>
        @empty
            <p class="p-3 text-center text-sm text-gray-600">
                No hay vacantes que mostar.
            </p> 
        @endforelse 
    </div>
